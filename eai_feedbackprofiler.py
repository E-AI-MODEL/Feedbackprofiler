...
rubric_VC = {
    (0.1, 0.2): {
        ...
        "advies": "Gebruik AI alleen als hulpmiddel bij begripsopbouw, niet als vervanger."
    },
    (0.5, 0.6): {
        "omschrijving": "AI ondersteunt verklarend denken en eenvoudige analyse",
        "micro_min": "Verklaringen via vergelijkingen",
        "micro_max": "Oorzaak-gevolgredenering in beperkte context",
        "advies": "Laat leerlingen eigen verklaringen formuleren in plaats van AI-responses kopiëren."
    },
    (0.9, 1.0): {
        ...
        "advies": "Begeleid leerlingen actief bij het doorgronden van complexe AI-output."
    }
}
...
rubric_VM = {
    (0.1, 0.2): {
        ...
        "advies": "Stimuleer leerlingen om zelf doelen en reflecties te formuleren."
    },
    (0.5, 0.6): {
    ...
    (0.9, 1.0): {
        ...
        "advies": "Zorg voor evenwicht tussen AI-coaching en menselijke begeleiding."
    }
}
...
td_matrix = {
    ("0.9", "V_C"): {"kernhandeling": "evalueren en generaliseren van kennis", "flag": "AI_dominantie", "norm": 0.3},
    ("0.5", "V_M"): {"kernhandeling": "bewustwording van eigen begrip", "flag": "Underuse_warning", "norm": 0.5}
}
...
if st.button("Analyseer feedback") and feedback.strip():
    ...
    # Gesimuleerde analyse (kan worden vervangen door GPT-inschatting)
    score_vc = 0.6
    score_vm = 0.6
    p_score = 0.9
    td_score = 0.65
    ...
    st.markdown(f"- Microdescriptoren: {vc_match['micro_min']} → {vc_match['micro_max']}")

    st.markdown(f"**V_M-score:** {score_vm} → {vm_match['omschrijving']}")
    st.markdown(f"- Microdescriptoren: {vm_match['micro_min']} → {vm_match['micro_max']}")

    st.subheader("2. Taakdichtheid (TD)")
    st.markdown(f"**TD-score**: {td_score} bij P={p_score}, V_C → Flag: `{td_flag_result}`")
    ...
    fig, ax = plt.subplots()
    ax.barh(["TD-score"], [td_score], color="orange" if td_score > 0.5 else "green")
    ax.set_xlim(0, 1)
    st.pyplot(fig)
